...
TEST(ConstructorsTests, DefaultConstructorTest1) {
  std::queue<int> testing1;
  s21::queue<std::string> testing2;
...
TEST(ConstructorsTests, DefaultConstructorTest2) {
  s21::queue<int> s1{5, 6, 8, 2, 10, 1};
  ASSERT_FALSE(s1.empty());
  ASSERT_EQ(s1.size(), 6);
}

TEST(ConstructorsTests, DefaultConstructorTest3) {
  s21::queue<int> s2{1, 2, 1};
  std::queue<int> s;
  s.push(1);
  s.push(1);
  s.push(1);
